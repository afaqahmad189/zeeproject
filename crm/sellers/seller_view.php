...
if (mysqli_num_rows($data) > 0) {
    ...
    $mail=$row['mail_id'];
	$other=$row['other'];
	$sellernum=$row['seller_phone'];
    $selleracc=$row['seller_account'];
	...
	<div class="row"> 
		...
		<div style="padding: 10px; max-width: 30%; " class="col card ">
			...
			<table class="table text-center">
                ...
                <?php
                if($type=='1'){
                    $simple_product="select *  from sellerinactive_products A,add_product B where A.seller_id='$seller_id' and A.inactive_pro_id=B.id order by A.id limit 3";
                    $result_simple_product=mysqli_query($conn,$simple_product);
                    if(mysqli_num_rows($result_simple_product)>0){
                        while($data_simple_product=mysqli_fetch_assoc($result_simple_product)){
                            $inactive_product_name=$data_simple_product['name'];
                            $inactive_product_price=$data_simple_product['inactive_pro_price'];
                            $inactive_product_phone=$data_simple_product['inactive_pro_phone'];

                            echo'<tr><td>'.$inactive_product_name.'</td><td>'.$inactive_product_price.'</td><td>orders</td></tr>';
                        }
                    }
                    else{
                        echo'<tr><td colspan="3">No Active Products</td></tr>';
                    }
		...
		<a style=" font-size: 24px; margin-bottom: 12px" class="btn btn-primary viewpagebtn" href="/zeeproject/crm/sellers/edit_seller.php?seller_id=<?php echo $seller_id  ?>">Edit Seller</a>
		

		<a style=" font-size: 24px" class="btn btn-warning" href="/zeeproject/crm/sellers/insert_khata.php?seller_id=<?php echo $seller_id  ?>">View Khata</a>	
		...
		<div style="padding: 20px; margin-left: 15px" class="col card ">
			...
			<table class="table text-center">
				...
				<tr>
                    ...
                    <?php
                    $sql_dealer= "select * from seller_dealer 
                    ...
                    if (mysqli_num_rows($data_dealer) > 0) {
                        while($row_dealer= mysqli_fetch_assoc($data_dealer)){
                            $dealer_name = $row_dealer['s_dealer_name'];
                            $dealer_contact = $row_dealer['s_contact_number'];
                            $dealer_account = $row_dealer['s_account_number'];
                            echo '<tr><td>'.$dealer_name.'</td><td>'.$dealer_contact.'</td><td>'.$dealer_account.'</td></tr>';
...
<br>
		<center><h4>Company Details</h4></center><br>
			<table class="table text-center">

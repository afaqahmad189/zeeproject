...
//        move_uploaded_file($_FILES["thirdpics"]["tmp_name"], $target_file_img3);
       ...
       if(mysqli_num_rows($getrow)>0){
          ...
          $sql="insert into add_product (name,barcode,affect ,cat_id,unit,qty,alert_stock,product_cost,sale_price,description,
                                            product_place,warehouse_num,pur_place,main_pic) 
                values('$productname','$barcode','$invetoryaffect','$catselect','$unit','$quantity','$alertstock','$productcost',
                        '$productcost','$description','$rack_no','$warehoue_num','$purchased_place','$target_file_img1')";
    ...
    case 'add_category':{
        ...
        if(mysqli_num_rows($row)>0){
            echo '<div class="col-md-8"></div><div class="col-md-4 toast" ><label>Already Created</label></div>';
            ...
            if(mysqli_query($conn,$query) or die(mysqli_error($conn))){
                echo '<div class="col-md-8"></div><div class="col-md-4 toast" ><label>Category Created</label></div>';
...
//                header('location:add_product.php');
    ...
    case'get_cat':{
        ...
        $row=mysqli_query($conn,$sql) or die(mysqli_error($conn));
        if(mysqli_num_rows($row)>0){
//            while ($data=mysqli_fetch_assoc_array($row)){
//                $cat_id=$data['id'];
//                $catname=$data['cat_name'];
//                echo '<option value="'.$cat_id.'">'.$catname.'</option>';
//            }
            $get=mysqli_fetch_assoc($row);
//            $data=$get['id'];
            $data=$get;
    ...
    case'update_product':{
        ...
        if($target_file_img1=='products/'){
            $sql="UPDATE add_product SET name ='$productname',barcode='$barcode',affect='$invetoryaffect',cat_id='$catselect',
                unit='$unit',qty='$quantity',alert_stock='$alertstock',product_cost='$productcost',sale_price='$saleprice',
                description='$description',product_place='$rack',warehouse_num='$warehoue_num',pur_place='$productplace'                                                          WHERE id='$product_id'";
            if(mysqli_query($conn,$sql)){
                //other_image_upload
                $total = count($_FILES['otherpics']['name']);
                ...
                if($total_3d >=1 && $tmpFilePathcheck_3d[0]!=""){
                    ...
                    if(mysqli_query($conn,$del_img_3d)or die(mysqli_error($conn))){
            ...
            $sql="UPDATE add_product SET name ='$productname',barcode='$barcode',affect='$invetoryaffect',cat_id='$catselect',
                unit='$unit',qty='$quantity',alert_stock='$alertstock',product_cost='$productcost',sale_price='$saleprice',
                description='$description',product_place='$rack',warehouse_num='$warehoue_num',pur_place='$productplace',main_pic='$target_file_img1'
                                                          WHERE id='$product_id'";
            if(mysqli_query($conn,$sql)){
                //other_image_upload
                $total = count($_FILES['otherpics']['name']);
